...
//     double b1 = Triangle2D( other, this->terminal() ).doubleSignedArea();
    double a0 = Triangle2D::double_signed_area( this->origin(), this->terminal(), other.origin() );//求三点形成的三角形面积
...
Segment2D::dist( const Vector2D & p ) const
{
    ...
    return std::sqrt( fmin( origin().dist2( p ),
                                terminal().dist2( p ) ) );
}
...
Segment2D::dist( const Segment2D & seg ) const
{
    ...
    return fmin( fmin( this->dist( seg.origin() ),
                               this->dist( seg.terminal() ) ),
                     fmin( seg.dist( origin() ),
                               seg.dist( terminal() ) ) );
}
...
{
    return std::sqrt( fmax( origin().dist2( p ),
                                terminal().dist2( p ) ) );
}
