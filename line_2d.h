...
public:
    ...
    */
    ...
    bool onLeft(const Vector2D& pos) const
    {
        return (-pos.y * M_b - M_c) / M_a > pos.x;
    }
    bool onRight(const Vector2D& pos) const
    {
        return (-pos.y * M_b - M_c) / M_a < pos.x;
    }
    bool above(const Vector2D& pos) const
    {
        return (-pos.x * M_a - M_c) / M_b < pos.y;
    }
    bool under(const Vector2D& pos) const
    {
        return (-pos.x * M_a - M_c) / M_b > pos.y;
    }
    ...
    */
    ...
    Vector2D intersection( const Line2D & line1,
                           const Line2D & line2 );
    Vector2D SymmetryVec(Vector2D& v)
    {
        Vector2D o(0, 0);
        return projection(v)*2-v-projection(o)*2;
    }
    Vector2D Symmetry(Vector2D v)
    {
        Vector2D o(0, 0);
        return projection(v) * 2 - v;
    }
