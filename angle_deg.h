...
public:
    ...
    * 将角度转化为90°以内以实现最快转动
    */
    AngleDeg& normalize_again()
    {
        while (M_degree > 180)M_degree -= 360;
        while (M_degree < -180)M_degree += 360;
        while (M_degree > 90)M_degree -= 180;
        while (M_degree < -90)M_degree += 180;
        return *this;
    }
    /*!
    ...
    */
    ...
    double atan2_deg( const double & y,
                      const double & x )
      {
          return ( ( x == 0.0 && y == 0.0 ) ? 0.0: rad2deg( std::atan2( y, x ) ) );//返回arctan（y/x）对应的角度
      }
    ...
    */
    ...
    std::ostream & printRound( std::ostream & os,
      ...
      {
		  double x = degree()/step;
          return os << rint( x ) * step;
      }*/
